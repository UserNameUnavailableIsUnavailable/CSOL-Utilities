...
)
{
    ...
    $ControllerConfig = [ordered]@{
        ...
        "Idle.EngineType" = "Classifier"
        "Idle.ModelConfigPath" = [System.IO.Path]::Combine($ControllerModelDirectory, "Classifier", "ResNet", "CSOL-Utilities-ResNet18-800x600.json")
        "Idle.StartGameRoomTimeout" = 900
        "Idle.LoginTimeout" = 300
        "Idle.MaxInGameTime" = 114514
        "Idle.LoadMapTimeout" = 5 * 60
        "Idle.RestartGameOnLoadingTimeout" = $True
        "Idle.SwitchToDefaultModeAfterReconnection" = $True
        "Idle.SuppressQuickFullscreen" = $True
        "Idle.SuppressCSOBanner" = $True
