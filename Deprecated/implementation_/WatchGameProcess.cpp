...
void Controller::WatchGameProcess() noexcept
{
	...
	Console::Log(ERROR_LEVEL::CUML_MESSAGE, "游戏启动命令：%ls", s_Instance->m_LaunchGameCmd.get());
	while (true)
	{
		...
		do
		{
			...
			if (game_process_state == GAME_PROCESS_STATE::GPS_BEING_CREATED)
			{
				...
				{
					Console::Log(ERROR_LEVEL::CUML_ERROR, "获取反恐精英 Online 进程标识符时发生错误。错误代码：%lu。",
								 GetLastError());
					break;
				}
				...
				if (!hGameProcess)
				{
					Console::Log(ERROR_LEVEL::CUML_ERROR, "尝试获取反恐精英 Online 进程信息时发生错误。错误代码：%lu。",
								 GetLastError());
					break;
				}
				game_process_state = GAME_PROCESS_STATE::GPS_RUNNING; /* 跳转执行监测游戏进程运行状态的代码块 */
				Console::Log(ERROR_LEVEL::CUML_MESSAGE, "成功获取游戏进程信息。进程标识符：%lu。", dwGameProcessId);
			...
			else if (game_process_state == GAME_PROCESS_STATE::GPS_RUNNING)
			{
				...
				{ /* 进程结束 */
					...
					game_process_state = GAME_PROCESS_STATE::GPS_UNKNOWN;
					Console::Log(ERROR_LEVEL::CUML_WARNING, "游戏进程退出。");
				...
				{ /* 等待失败 */
					Console::Log(ERROR_LEVEL::CUML_ERROR, "m_GameProcessWatcher 运行遇到错误。错误代码：%lu。\r\n",
								 GetLastError());
					break;
				}
			}
			...
			{ /* 游戏进程退出，重启游戏 */
				...
				if (bRet)
				{
					Console::Log(ERROR_LEVEL::CUML_MESSAGE, "等待游戏进程启动。");
				}
				else
				{
					Console::Log(ERROR_LEVEL::CUML_WARNING,
								 "通过 TCGame 自动创建游戏进程失败。错误代码：%lu。请尝试手动运行游戏。",
								 GetLastError());
				}
			...
			{ /* 游戏进程状态未确定，确定游戏进程状态 */
				...
				Console::Log(ERROR_LEVEL::CUML_MESSAGE, "游戏进程状态未知，检测游戏进程状态。");
				...
				Console::Log(ERROR_LEVEL::CUML_MESSAGE, "成功获取游戏进程信息。游戏进程标识符：%lu。", dwGameProcessId);
	...
	Console::Log(ERROR_LEVEL::CUML_MESSAGE, "线程 m_GameProcessWatcher 退出。");
