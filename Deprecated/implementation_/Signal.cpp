...
namespace CSOL_Utilities
{
	...
	std::shared_ptr<LogicElement> Signal::dispatch(std::unique_lock<std::mutex>& lock)
	{
		...
		m_requesters.erase(std::remove_if(m_requesters.begin(), m_requesters.end(),
										  [](std::weak_ptr<LogicElement>& t)
										  {
											  auto tok = t.lock();
											  if (!tok || tok->get_state() != LogicElement::m_SLEEPING)
											  {
												  return true;
											  }
											  return false;
	...
	bool Signal::deliver(std::shared_ptr<LogicElement> token)
	{
		if (!token)
		{
			return false;
	...
	void Signal::reserve(std::shared_ptr<LogicElement> token)
	{
	...
	bool Pin::wake(std::shared_ptr<Signal> signal)
	{
