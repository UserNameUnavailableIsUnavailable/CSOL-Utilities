...
ModuleBase::ModuleBase(std::string name, unsigned int message_queue_capacity) :
	m_name(name), m_MESSAGE_QUEUE_CAPACITY(message_queue_capacity), m_id(++s_module_counter) // id 分配时从 1 开始
{
}
...
void ModuleBase::rest()
{
	std::unique_lock lock(m_worker_state_mutex);
	// 老板良心发现
	m_can_suspend.wait(lock,
					   [this]
					   {
						   return m_task_finished_by_worker || m_sacked || m_retired ||
							   m_dead; // 工作完成，则可以让打工人休息；或者，打工人被开除、退休、猝死
					   });
...
{
	std::unique_lock lock(m_worker_state_mutex);
	m_can_suspend.wait(lock, [this] { return m_task_finished_by_worker || m_sacked || m_retired || m_dead; });
...
std::unordered_map<unsigned int, std::weak_ptr<ModuleBase>> ModuleBase::s_MODULE_TABLE =
	std::unordered_map<unsigned int, std::weak_ptr<ModuleBase>>();
...
{
	ModuleMessage module_message{.id = m_id, .message = message};
	...
	boss_module->m_message_queue_not_full.wait(
		lock, [&] { return boss_module->m_message_queue.size() < m_MESSAGE_QUEUE_CAPACITY; });
...
bool ModuleBase::send_message_to_employee(MODULE_MESSAGE_T message, unsigned int employee_module_id)
{
	...
	employee_module->m_message_queue_not_full.wait(
		lock, [&] { return employee_module->m_message_queue.size() < m_MESSAGE_QUEUE_CAPACITY; });
...
void ModuleBase::recruit(unsigned int id)
{
	...
	employee->m_can_suspend.wait(lock,
								 [employee]
								 {
									 return employee->m_task_finished_by_worker || employee->m_sacked ||
										 employee->m_retired || employee->m_dead;
								 });
	// 打工人是否已经有老板，兼职的不要
