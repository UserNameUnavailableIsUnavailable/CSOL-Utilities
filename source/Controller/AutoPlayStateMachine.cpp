...
/* 游戏进行中可能被检测到的文本 */
const char* MAP_TEXT[]{ "录制", "录像", "视频", "取消", "选择角色", "剩余时间", "自动选择", "下一个", "延迟时间", "选择武器", "重新购买", "开启提示", "僵尸的战利品", "无法携带更多", "连续杀敌", "全员杀敌", "无法丢弃该武器", "通关失败", "使用回合重置道具", "购买菜单", "金币总计", "胜率", "成功通关", "通关时间", "已成功完成一局游戏", "胜率", "当前关数" };
...
void Controller::DispatchAutoPlayCommand()
{
    ...
    else if (m_CurrentState.GetState() == IN_GAME_STATE::IGS_IN_MAP) /* 游戏中 */
    {
        ...
        else if (current_time - m_CurrentState.GetTimestamp() > 7)
        {
