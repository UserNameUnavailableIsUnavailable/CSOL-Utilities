...
class Console
{
  public:
    ...
    static void Log(EXCEPTION_LEVEL level, std::string s) noexcept;
    template <typename... VARARG>
    static void Log(EXCEPTION_LEVEL level, _Printf_format_string_ const char *fmt, const VARARG &...args) noexcept
    {
        ...
        if (level == EXCEPTION_LEVEL::EL_MESSAGE)
        {
        ...
        else if (level == EXCEPTION_LEVEL::EL_DEBUG)
        {
            std::printf("\x1b[94m%s【调试】", current_time_string);
            std::printf(_Printf_format_string_ fmt, args...);
            std::printf("\x1b[39m\r\n");
        }
        else if (level == EXCEPTION_LEVEL::EL_WARNING)
        {
            std::printf("\x1b[93m%s【警告】", current_time_string); /* 黄色字体打印警告 */
            std::printf(_Printf_format_string_ fmt, args...);
            std::printf("\x1b[39m\r\n");
        }
        else if (level == EXCEPTION_LEVEL::EL_ERROR)
        {
