...
namespace CSOL_Utilities
{
    ...
    //        Console::Log(CSOL_UTILITIES_MESSAGE_LEVEL::EL_DEBUG, "本次截图耗时：%llu。", capture_elapse);
    //#endif // _DEBUG
        }
        ...
        Console::Log(CSOL_UTILITIES_MESSAGE_LEVEL::EL_DEBUG, "本次识别耗时：%llu", recognition_elapse);
#endif
        ...
        Console::Log(CSOL_UTILITIES_MESSAGE_LEVEL::EL_DEBUG, "本次解析耗时：%llu", parse_elapse);
#endif
        ...
        {
            state.update(IN_GAME_STATE::IGS_IN_HALL, current_time);
            Console::Log(EXCEPTION_LEVEL::EL_WARNING, "在房间内等待时间超过预设 %u 秒，回到大厅重新创建房间。", s_Instance->GetMaxWaitTimeInGameRoom());
        }
        ...
        {
            Console::Log(EXCEPTION_LEVEL::EL_WARNING, "等待游戏加载达到超时时间（超时设定为 150 秒，超过该时间可能是游戏失去响应），结束游戏进程以重新启动。");
            std::lock_guard lk(s_Instance->m_GameInfoMutex);
    ...
    void Controller::DispatchAutoPlayCommand()
    {
        ...
        if (m_CurrentState.GetState() == IN_GAME_STATE::IGS_UNKNOWN)
        {
            ...
            cmd.Set(COMMAND::CMD_CLEAR_POPUPS, current_time);
        ...
        else if (m_CurrentState.GetState() == IN_GAME_STATE::IGS_IN_HALL) /* 大厅内 */
        {
            ...
            cmd.Set(COMMAND::CMD_CREATE_GAME_ROOM, current_time);
        ...
        else if (m_CurrentState.GetState() == IN_GAME_STATE::IGS_IN_ROOM) /* 房间内（正常） */
        {
            ...
            cmd.Set(COMMAND::CMD_START_GAME_ROOM, current_time);
            remove_game_window_border();
            s_Instance->m_Messenger.Dispatch(cmd);
        }
        ...
        else if (m_CurrentState.GetState() == IN_GAME_STATE::IGS_IN_MAP) /* 游戏中 */
        {
            ...
            {
                cmd.Set(COMMAND::CMD_EXTENDED_IDLE, current_time);
            }
            else if (current_time - m_CurrentState.GetTimestamp() > 5)
            {
                cmd.Set(COMMAND::CMD_DEFAULT_IDLE, current_time);
            }
            else
            {
                cmd.Set(COMMAND::CMD_CHOOSE_CHARACTER, current_time);
            }
            remove_game_window_border();
            s_Instance->m_Messenger.Dispatch(cmd);
        }
    ...
    void Controller::WatchInGameState() noexcept
    {
        ...
        {
            Console::Log(EXCEPTION_LEVEL::EL_ERROR, "%s", e.what());
        }
        Console::Log(EXCEPTION_LEVEL::EL_MESSAGE, "线程 m_InGameStateWatcher 退出。");
