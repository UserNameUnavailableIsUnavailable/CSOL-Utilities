...
namespace CSOL_Utilities
{
    ...
    void CircuitModule::entry() // 工作线程入口
    {
        while (true)
        {
            ...
            m_should_worker_resume.wait(lock, [this] {
                return !m_latches_resolved || m_worker_state == 1 || m_worker_state == -1; // 锁存器更新、正常（1）、异常（-1）
            });
    ...
    void CircuitModule::in()
    {
        ...
        // - 非暂停状态
        m_should_worker_resume.wait(lock, [this] {
            return !m_latches_resolved || m_worker_state != 0;
        });
        if (!m_latches_resolved) // 如果锁存器信号变更，则根据信号计算出工作状态
        {
    ...
    void CircuitModule::out(int signal)
    {
        ...
        if (next_element) // 下一级元件存在
        {
            // 出作用域解锁
            {
                ...
                next_element->m_can_worker_suspend.wait(lock, [&] {
        ...
        // if (signal == -1 || signal && latch->mask) // 异常或正常（即不是挂起），则说明下一级元件也许能够运行，这时需要尝试将其唤醒
        // {
        //     m_should_worker_resume.notify_one(); // 传入新信号之后，下一级可能处于挂起态（不接收锁存器输入），将其唤醒
        // }
    }
