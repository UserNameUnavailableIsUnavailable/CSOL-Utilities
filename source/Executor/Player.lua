...
then
    ...
    function Player:reset_round_or_respawn()
        Runtime:sleep(20, true)
        local x, y = Mouse:locate_cursor()
        Mouse:move_relative(250, 20, 10)
        local _x, _y = Mouse:locate_cursor()
        if (math.abs(_x - x) < 200 and math.abs(_y - y) < 200 and math.abs(_x - 32767) < 250 and math.abs(_x - 32767) < 250)
        then
            Keyboard:click_several_times(self.RESPAWN_KEY, 2, Delay.MINI)
            return
        end
        ...
        Keyboard:click_several_times(self.RESPAWN_KEY, 2, Delay.MINI) -- 回合重置
    end
