declare var luaparse /* luaparse 在 html 中引入 */
declare var g_WeaponTemplateList /* 在 html 的 js 块中定义 */
...
export class ConventionalWeapon extends Weapon {
    ...
    m_ReloadingNeeded: boolean = true // 是否需要重新装填
    m_SwitchDelayElement: HTMLInputElement
    m_AttackButtonElements: Array<HTMLInputElement>
    m_ReloadingNeededElements: Array<HTMLInputElement>
    ...
    constructor() {
        ...
        p = document.createElement('p')
        label = document.createElement("label")
        label.innerHTML = "是否需要重新装填"
        p.appendChild(label)
        this.m_ReloadingNeededElements = new Array<HTMLInputElement>
        input = document.createElement("input")
        input.name = `Weapon_${this.m_Id}_reloading_needed`
        this.m_ReloadingNeededElements.push(input)
        input.value = "true"
        input.type = "radio"
        input.checked = true
        label = document.createElement("label")
    ...
    override gather(): void {
        ...
        let reloading_needed = document.querySelector(`input[name='Weapon_${this.m_Id}_reloading_needed']:checked`) as HTMLInputElement
        this.m_ReloadingNeeded = reloading_needed.value === "true" ? true : false
    }
    override generate(indent: string, indent_level: number = 0, first_line_indent: boolean = true): string {
        this.gather()
        return `${first_line_indent ? indent.repeat(indent_level) : ""}Weapon:new({\n` +
        ...
        `${indent.repeat(indent_level + 1)}reloading_needed = ${this.m_ReloadingNeeded}\n` +
        `${indent.repeat(indent_level)}})`
    }
    override async update(): Promise<void> {
        ...
        this.m_ReloadingNeededElements.forEach(e => {
            if (e.value === `${this.m_ReloadingNeeded}`) {
                e.checked = true
            }
        })
    }
...
export class CustomizedWeapon extends Weapon {
    ...
    constructor(template_class: string) {
        ...
        this.m_TemplateCode = "Weapon:new({})"
        let p = document.createElement('p')
    ...
    override generate(indent: string, indent_level: number = 0, first_line_indent: boolean = true): string {
        ...
        let user_defined_fields = ast.body[0].expression.arguments[0].fields
        for (let i = 0; i < user_defined_fields.length; i++) {
            // user_defined_fields[i]: TableKeyString
            // key, value
            const key = LuaASTUtil.GenerateLuaCodeFromAST(user_defined_fields[i].key)
            args.set(key, user_defined_fields[i])
        }
        let fields = new Array()
        args.forEach(e => {
            fields.push(e)
        })
        ast.body[0].expression.arguments[0].fields = fields
...
function __impl_resolve_weapon(ast) {
    ...
    for (const field of args.fields) {
        ...
        } else if (LuaASTUtil.GenerateLuaCodeFromAST(field.key) === "reloading_needed") {
            fields.set("reloading_needed", LuaASTUtil.GenerateLuaCodeFromAST(field.value))
...
export function ResolveWeaponList(ast) {
    let armor: Armor
    let default_part_weapons = new Array<PartWeapon>()
    let extended_part_weapons = new Array<PartWeapon>()
    let default_conventional_weapons = new Array<ConventionalWeapon | CustomizedWeapon>()
    let extended_conventional_weapons = new Array<ConventionalWeapon | CustomizedWeapon>()
    let default_special_weapons = new Array<CustomizedWeapon>()
    let extended_special_weapons = new Array<CustomizedWeapon>()
                ...
                if (variable === "Armor" || variable === "AC") {
                    ...
                    for (let i = 0; i < weapons.length; i++) {
                        ...
                        if (dict.has("template_name") || dict.has("template_code")) {
                            ...
                            weapon.m_ReloadingNeeded = dict.get("reloading_needed") === "true" ? true : false
