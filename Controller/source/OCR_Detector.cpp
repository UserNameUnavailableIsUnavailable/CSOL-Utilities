...
std::vector<std::array<cv::Point, 4>> OCR_Detector::MakeDetectionResult(const cv::Mat &possibility_map,
...
std::vector<std::array<cv::Point, 4>> OCR_Detector::Run(const cv::Mat &image)
{
    ...
    auto raw_output_tensor =
        results.front().GetTensorData<float>(); /* 模型输出概率图，对应于输入图像的每个像素点，指示其属于文字符号的概率
                                                 */
