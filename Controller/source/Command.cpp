...
void Command::Get(std::string &s)
{
    ...
    auto new_size = std::formatted_size(DIRECTIVES_TEMPLATE, self.id_, QueryCommandString(self.type_),
                                        static_cast<int64_t>(std::chrono::system_clock::to_time_t(self.timepoint_)),
                                        self.repeatable_);
    s.resize(new_size);
...
void Command::Set(Command::TYPE cmd_type, Command::MODE mode)
{
    ...
    if (self.type_ == cmd_type && /* 连续下达同一命令 */
        std::chrono::duration_cast<std::chrono::milliseconds>(now - self.timepoint_) <
            QueryCommandCoolDownTime(cmd_type))
    {
        return; /* 冷却时间尚未结束 */
    }
    ...
    self.id_++;
    self.type_ = cmd_type;
    self.timepoint_ = static_cast<bool>(mode & CMD_ZERO_TIMESTAMP)
