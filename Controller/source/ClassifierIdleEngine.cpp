...
void ClassifierIdleEngine::Run()
{
	...
	Console::Debug(std::format("本轮推理耗时 {} 毫秒", std::chrono::duration_cast<std::chrono::milliseconds>(classify_end - classify_start).count()));
	#endif
	...
	case GAME_INTERFACE_TYPE::LOBBY:
		{
			...
			case GAME_INTERFACE_TYPE::LOGIN: // LOBBY -> LOGIN
				{
					auto msg = std::format("Unexpected transition from {} to GAME_INTERFACE_{}. File {}, line {}.",
										   InterfaceTypeToString(interface_type),
										   InterfaceTypeToString(current_interface_type), __FILE__, __LINE__);
			...
			case GAME_INTERFACE_TYPE::IN_GAME: // IN_GAME -> IN_GAME
				{
					...
					else if (elapsed < std::chrono::seconds(120 + 7)) // 进入游戏后的 120 秒内用默认模式挂机
					{
						command_type_ = Command::TYPE::CMD_DEFAULT_IDLE_2;
							 ...
							 std::chrono::seconds(Global::MaxInGameTime)) // 超过 120 秒且为扩展模式，执行扩展挂机
					{
