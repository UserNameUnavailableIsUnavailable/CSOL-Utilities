...
namespace CSOL_Utilities
{
	...
	cv::Mat PadImage(cv::Mat& img, int padding)
	{
		...
		cv::Mat padded_img;
		cv::copyMakeBorder(img, padded_img, padding, padding, padding, padding, cv::BORDER_ISOLATED, paddingScalar);
		return padded_img;
	}
	...
	float BoxScoreFast(const std::vector<cv::Point2f>& boxes, const cv::Mat& pred)
	{
		...
		int minX = std::clamp(int(std::floor(*(std::min_element(arrayX, arrayX + 4)))), 0, width - 1);
		int maxX = std::clamp(int(std::ceil(*(std::max_element(arrayX, arrayX + 4)))), 0, width - 1);
		int minY = std::clamp(int(std::floor(*(std::min_element(arrayY, arrayY + 4)))), 0, height - 1);
		int maxY = std::clamp(int(std::ceil(*(std::max_element(arrayY, arrayY + 4)))), 0, height - 1);
	...
	std::vector<cv::Point2f> GetMinBoxes(const cv::RotatedRect& boxRect, float& maxSideLen)
	{
		...
		std::sort(boxPoint.begin(), boxPoint.end(), [](const cv::Point& a, const cv::Point& b) { return a.x < b.x; });
		int index1, index2, index3, index4;
	...
	cv::RotatedRect Unclip(std::vector<cv::Point2f> box, float unClipRatio)
	{
		...
		ClipperLib::Path p;
		p << ClipperLib::IntPoint(int(box[0].x), int(box[0].y)) << ClipperLib::IntPoint(int(box[1].x), int(box[1].y))
		  << ClipperLib::IntPoint(int(box[2].x), int(box[2].y)) << ClipperLib::IntPoint(int(box[3].x), int(box[3].y));
