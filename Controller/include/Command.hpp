...
class Command
{
  public:
    ...
    static constexpr std::string_view NOP()
    {
        return "Command.directives={id=0,type=Command.CMD_NOP,timepoint=0,"
               "repeatable=false}";
    }
    ...
    static constexpr std::chrono::milliseconds QueryCommandCoolDownTime(Command::TYPE cmd)
    {
        switch (cmd)
        {
        /* 创建一个房间的耗时在 15 秒左右，进行一次状态判定的耗时从 1 ~ 6
         * 秒不等，具体取决于硬件配置 */
        /* 假定某一段时间内状态判定耗时均为 τ 秒，那么为了使申请到新 ID
         * 时的判定的状态是准确的，则： */
  ...
  private:
    ...
    std::chrono::time_point<std::chrono::system_clock> timepoint_ =
        std::chrono::system_clock::time_point(); /* 命令下达时刻 */
    static constexpr std::string_view DIRECTIVES_TEMPLATE =
        "Command.directives={{id={}, type={},timepoint={},repeatable={}}}"; /* 指令模板
                                                                             */
};
} // namespace CSOL_Utilities
