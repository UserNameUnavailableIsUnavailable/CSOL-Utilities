...
{
  public:
    OCR(OCR_Detector &&detector, OCR_Recognizer &&recognizer);
...
std::vector<float> SubstractMeanNormalize(const cv::Mat &image, const std::array<float, N> &mean,
                                          const std::array<float, N> &norm)
{
    ...
    for (size_t pixel_index = 0; pixel_index < image_size; pixel_index++)
    {
        ...
        {
            float data = image.data[pixel_index * num_channels + channel_index] * norm[channel_index] -
                         mean[channel_index] * norm[channel_index];
