...
{
    template <std::size_t N> friend void WaitForMultipleEvents(std::shared_ptr<Event> (&events)[N], bool wait_all);
    template <std::size_t N, typename Rep, typename Period>
    friend bool WaitForMultipleEvents(std::shared_ptr<Event> (&events)[N], bool wait_all,
  ...
  public:
    ...
    template <typename Rep, typename Period> bool Wait(std::chrono::duration<Rep, Period> duration) const
    {
...
template <std::size_t N, typename Rep, typename Period>
    requires WAIT_EVENTS_MAX<N>
bool WaitForMultipleEvents(std::shared_ptr<Event> (&events)[N], bool wait_all,
                           std::chrono::duration<Rep, Period> duration)
{
    HANDLE hEvent[N];
    for (std::size_t i = 0; i < N; i++)
