---魔神开天（Twin Light Axes）模板
---@class TwinLightAxes : Weapon
---@field last_light_vortex_time integer 上一次释放大回旋的时刻
Weapon:new({
    name = "魔神开天",
    switch_delay = Delay.LONG,
    number = Weapon.MELEE,
    purchase_sequence = {},
    last_light_vortex_time = 0,
    template_name = "TwinLightAxes",
    ---@param self TwinLightAxes
    ---@param round integer
        ...
        if round > 0 then
            return function()
                ...
                if tp - self.last_light_vortex_time > 6000 then
                    ...
                    Keyboard:press(self:get_reload_key(), 200) -- 按下左键的同时再按下 R 键触发大回旋
                    Keyboard:release(self:get_reload_key())
