...
const normal_weapon_remarks = [
    "J 键可切换 T / CT 阵营武器购买界面。",
    "攻击持续时间取值一般在 7.5 ~ 25 秒，武器消耗完一个弹夹的时间可作为选择依据（对于近战武器以及没有传统意义上弹夹的武器，此处的攻击时间可适当延长，但不建议超过 30 秒）。弹夹消耗完后，将丢弃当前武器，并从武器列表选择下一件武器，购买后进行攻击。对于天驱、擎空等子弹耗尽时不需要按 R 即可自动换弹的武器，可适当延长其攻击时间至 30 秒。",
    "扫射方向即使用武器攻击时视角的运动方向，分解为水平和垂直两个方向。水平方向一般选为随机，垂直方向按实际需求进行调整。例如，刷狂戮巨蚊、鹞子风筝时可选为固定向上。",
];

const customized_weapon_remarks = [
    "J 键可切换 T / CT 阵营武器购买界面。",
    "攻击持续时间取值一般在 7.5 ~ 25 秒，武器消耗完一个弹夹的时间可作为选择依据（对于近战武器以及没有传统意义上弹夹的武器，此处的攻击时间可适当延长，但不建议超过 30 秒）。弹夹消耗完后，将丢弃当前武器，并从武器列表选择下一件武器攻击。",
